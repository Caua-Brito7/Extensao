...
library(readr)
agua_e_esgoto_município_2015 <- read_delim("agua e esgoto - município - 2015.csv", 
                                           delim = ";", escape_double = FALSE, trim_ws = TRUE)
View(agua_e_esgoto_município_2015)

dados_sinisa <- agua_e_esgoto_município_2015
# PASSO 2: CORREÇÃO DOS PONTOS DE MILHAR
# ------------------------------------------------------------------------------
# Remove os pontos "." que o R confunde com decimais e converte para número real.

# Tratamento para a coluna de Água
dados_sinisa$POPR_RA <- gsub(".", "", dados_sinisa$POPR_RA, fixed = TRUE)
dados_sinisa$POPR_RA <- as.numeric(dados_sinisa$POPR_RA)

# Tratamento para a coluna de Esgoto
dados_sinisa$POPR_RE <- gsub(".", "", dados_sinisa$POPR_RE, fixed = TRUE)
dados_sinisa$POPR_RE <- as.numeric(dados_sinisa$POPR_RE)


# ------------------------------------------------------------------------------
# PASSO 3: FILTRAGEM PRO PARÀ
# ------------------------------------------------------------------------------

sinisa_pa <- dados_sinisa[dados_sinisa$Estado == "PA", ]


# ------------------------------------------------------------------------------
# PASSO 4: MONTAGEM E ESTRUTURAÇÃO DO ARQUIVO FINAL
# ------------------------------------------------------------------------------
# Criamos o data.frame exatamente com as 5 colunas exigidas pelo enunciado.

# Criando a tabela final puxando o ano pela 4ª coluna do banco
SINISA_UF <- data.frame(
  ANO       = sinisa_pa[, 4],                   # Puxa a 4ª coluna (Ano), sem erro de acento!
  NIVEL     = rep("MUNICIPIO", nrow(sinisa_pa)), # Repete "MUNICIPIO" 144 vezes
  CODMUNRES = sinisa_pa$CODMUNRES,
  POPR_RA   = sinisa_pa$POPR_RA,
  POPR_RE   = sinisa_pa$POPR_RE,
...
View(SINISA_UF)
write.csv(SINISA_UF, "SINISA_PA.csv", row.names = FALSE)





# TAREFA 3: PROCESSAMENTO DOS DADOS DO ATLAS (IDH) 
# ==============================================================================
...
idh_municipios <- read.csv("IDHM - 2010 - municípios - Atlas Brasil.csv", sep = ";", stringsAsFactors = FALSE)
idh_estado     <- read.csv("IDHM - 2010 (CENSO) e 2015 (PNAD) - total e por sexo - UF - Atlas Brasil.csv", sep = ";", stringsAsFactors = FALSE)
codigos_ibge   <- read.csv("códigos dos municípios - 2010.csv", sep = ";", stringsAsFactors = FALSE)


# ------------------------------------------------------------------------------
# PASSO 2:
# ------------------------------------------------------------------------------
# Transforma, por exemplo, "Abaetetuba (PA)" em apenas "Abaetetuba" para igualar ao banco do IBGE
idh_municipios$município <- gsub(" \\([A-Z]{2}\\)", "", idh_municipios$município)


# ------------------------------------------------------------------------------
# PASSO 3: EXTRAÇÃO DOS VALORES FIXOS DA UF (DA TABELA DE ESTADO)
# ------------------------------------------------------------------------------
# Buscamos a linha do estado do Pará na tabela de UF
para_dados <- idh_estado[idh_estado$UF == "Pará", ]

# Guardamos os valores de 2010 Masculino e Feminino 
valor_masc <- para_dados$IDHM_2010_M  
valor_fem  <- para_dados$IDHM_2010_F  


# ------------------------------------------------------------------------------
# PASSO 4: O MERGE 
# ------------------------------------------------------------------------------
# Cruzamos as tabelas pela coluna comum "município" 
idh_completo <- merge(idh_municipios, codigos_ibge, by = "município", all.x = TRUE)
...
# ------------------------------------------------------------------------------
# FILTRAGEM ESTADO
idh_completo$UF_DIGITO <- substr(as.character(idh_completo$CODMUNRES), 1, 2)
atlas_pa <- idh_completo[idh_completo$UF_DIGITO == "15", ]


# ------------------------------------------------------------------------------
# PASSO 6: MONTAGEM E EXPORTAÇÃO
# ------------------------------------------------------------------------------
ATLAS_UF <- data.frame(
  ANO       = 2010,
  NIVEL     = "MUNICIPIO",
  ...
  IDHM_CA   = atlas_pa$IDHM_2010, 
  IDHM_CA_M = valor_masc,          # Injeta o valor fixo masculino do estado
  IDHM_CA_F = valor_fem            # Injeta o valor fixo feminino do estado
)



View(ATLAS_UF)
write.csv(ATLAS_UF, "ATLAS_UF.csv", row.names = FALSE)
